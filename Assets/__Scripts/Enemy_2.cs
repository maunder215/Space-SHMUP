...
public class Enemy_2 : Enemy
{
    ...
    void Start()
    {
        ...
        if (Random.value > 0.5f) {
            // Setting up the .x of each point to it's negative will move it to
            // the other side of the screen
            p0.x *= -1;
            p1.x *= -1;
        }

        // Set the birthTime to the currect time
        birthTime = Time.time;
    ...
    public override void Move() {
        ...
        // If u>1, then it has been longer than the lifeTime since birthTime
        if (u > 1) {
        ...
        // Adjust u by adding a U Curve based on the Sine wave
